...
PPE_STR_JOIN_IMP_AGAIN:

    va_copy(cp, *args);

    act = va_arg(cp, ppe_str_join_action);
    if (act == PPE_STR_JOIN_END) {
    ...
    do {
        switch (act) {
            ...
            case PPE_STR_JOIN_SET_DELIMITER_CSTR:
                ...
                dsz = ppe_cs_size(d);
                goto PPE_STR_JOIN_IMP_NONE_ITEM;
